...
model CraneCrabB "A planar damped crane crab in the 3D world - case b)"
  ...
  MB.Parts.Body body1(r_CM=zeros(3), m=1) annotation (Placement(transformation(
        ...
        rotation=90,
        origin={0,-20})));
  Parts.Body body2(m=1, I=0.1) annotation (Placement(transformation(extent={{70,10},{90,30}})));
  ...
  MB.Joints.Prismatic prismatic2(
    ...
    useAxisFlange=true,
    n={1,0,0}) annotation (Placement(transformation(extent={{20,10},{40,30}})));
  ...
  Interfaces.PlanarTo3D adaptor3D annotation (Placement(transformation(extent={{44,14},{56,26}})));
equation
